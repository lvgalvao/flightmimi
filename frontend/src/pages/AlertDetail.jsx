...
const cabinLabels = { economy: 'Economica', premium_economy: 'Premium', business: 'Business', first: 'Primeira' };
...
export default function AlertDetail() {
  ...
  async function handleCheck() {
    setChecking(true);
    try { await api.checkAlertNow(id); await loadDetail(); } catch (err) { console.error('Check failed:', err); } finally { setChecking(false); }
  }
  ...
  async function handleExport() {
    try {
      ...
      const a = document.createElement('a'); a.href = url; a.download = `alert-${id}-history.csv`; a.click(); window.URL.revokeObjectURL(url);
    } catch (err) { console.error('Export failed:', err); }
  }

  async function handleDelete() {
    if (!confirm('Tem certeza que deseja deletar este alerta?')) return;
    await api.deleteAlert(id); navigate('/');
  }

  async function handleSaveTarget() {
    const price = parseFloat(newTarget);
    if (!price || price <= 0) return;
    await api.updateAlert(id, { target_price: price }); setEditingTarget(false); await loadDetail();
  }

  if (loading) return <div className="flex items-center justify-center py-24"><div className="w-10 h-10 spinner" /></div>;
