...
export default function PriceChart({ priceHistory, targetPrice }) {
  if (!priceHistory || priceHistory.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 sm:py-16">
        <div className="w-12 h-12 rounded-full flex items-center justify-center mb-4" style={{ background: 'var(--color-rose-wash)' }}>
  ...
  return (
    <div className="animate-fade-in -mx-2 sm:mx-0">
      <ResponsiveContainer width="100%" height={280}>
        <ComposedChart data={data} margin={{ top: 10, right: 10, left: 0, bottom: 5 }}>
          <defs>
            <linearGradient id="priceGradient" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="var(--color-rose-light)" stopOpacity={0.12} />
              <stop offset="95%" stopColor="var(--color-rose-light)" stopOpacity={0} />
            </linearGradient>
          </defs>
          <CartesianGrid strokeDasharray="3 3" stroke="var(--color-border-light)" vertical={false} />
          <XAxis dataKey="label" tick={{ fontSize: 10, fill: 'var(--color-muted)', fontFamily: 'Plus Jakarta Sans' }} axisLine={{ stroke: 'var(--color-border)' }} tickLine={false} />
          <YAxis domain={[minPrice, maxPrice]} tickFormatter={(v) => `R$${(v / 1000).toFixed(1)}k`} tick={{ fontSize: 10, fill: 'var(--color-muted)', fontFamily: 'Plus Jakarta Sans' }} axisLine={false} tickLine={false} width={55} />
          <Tooltip content={<CustomTooltip />} />
          <ReferenceLine y={targetPrice} stroke="var(--color-green)" strokeDasharray="6 4" strokeWidth={1.5} label={{ value: `Alvo R$${targetPrice.toLocaleString('pt-BR')}`, fill: 'var(--color-green)', fontSize: 10, fontFamily: 'Plus Jakarta Sans', fontWeight: 600 }} />
          <Area type="monotone" dataKey="price" fill="url(#priceGradient)" stroke="none" />
          <Line type="monotone" dataKey="price" stroke="var(--color-rose)" strokeWidth={2.5} dot={{ r: 4, fill: 'white', stroke: 'var(--color-rose)', strokeWidth: 2 }} activeDot={{ r: 6, fill: 'var(--color-rose)', stroke: 'white', strokeWidth: 2 }} />
        </ComposedChart>
      </ResponsiveContainer>
    </div>
  );
